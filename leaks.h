...
static inline void* __malloc(size_t size, const char* file, int line, const char* function)
{
	int fd = open("adrress_logs.py", O_CREAT | O_RDWR, 0644);
	FILE *log = fopen("adrress_logs.py", "a");
    void* ptr = malloc(size);
    if (ptr != NULL)
        fprintf(log,"Memory allocated at address: %p, file: %s, line: %d, fun: %s\n", ptr, file, line, function);
    else
        fprintf(log, "Failed to allocate memory at file: %s, line: %d, fun: %s\n", file, line, function);
	fclose(log);
	close(fd);
    return ptr;
}
